...
/****** Object:  StoredProcedure [dbo].[RELTEMPAUDITORIA]    Script Date: 02/01/2023 12:22:24 ******/
...
DELETE FROM RtPago WHERE RtPagoEstCod = @ESTCOD;
DELETE FROM RtPagoBnf WHERE RtPagoBnfEstCod = @ESTCOD;
DELETE FROM RtAberto WHERE RtAbertoEstCod = @ESTCOD;
DELETE FROM RtAjuste WHERE RTAJUSTEESTCOD = @ESTCOD;
DELETE FROM RTANALITICO WHERE RTANALITICOESTCOD = @ESTCOD;
DELETE FROM RTCONSOLIDADO WHERE RTCONSOLIDADOESTCOD = @ESTCOD;
DELETE FROM RTCANCELADO WHERE RTCANCELADOESTCOD = @ESTCOD;
DELETE FROM RTCONCILIADO WHERE RTCONCILIADOESTCOD = @ESTCOD;
DELETE FROM RtAntecipacao WHERE RtAntecipacaoEstCod = @ESTCOD;
...
/*ANALITICO VALOR PAGO COM AJUSTE (V2) - INICIO*/
INSERT INTO RtPago (RtPagoEstCod, RtPagoNsu, RtPagoAutCod, RtPagoDataTrn, RtPagoProduto,
RtPagoBandeira, RtPagoTrnCod, RtPagoValorTrn, RtPagoValorLIQEST, RtPagoValor, RtPagoDataInc, RtPagoGuid)
SELECT @ESTCOD, A.MovTrnNsu, A.MovTrnAutCodStr, A.MovTrnDta, 
A.MovTrnTipPrd, A.MovTrnBan, A.MovTrnCod, A.MovTrnVlr, A.MovTrnVlrLiqEst, B.VALORPAGO, @DATAINC, @RELGUID
FROM MovTrn01 A
INNER JOIN
(SELECT SUM(VlpVlrPag) VALORPAGO, VlpNsu, VlpAutCodStr, VlpTrnCod FROM VLRPAG
...
AND A.MovTrnAutCodStr = B.VlpAutCodStr
AND A.MovTrnCod <> B.VlpTrnCod;
/*ANALITICO VALOR PAGO COM AJUSTE (V2) - FIM*/
...
/*ANALITICO VALOR ABERTO COM AJUSTE (V2) - INICIO*/
INSERT INTO RtAberto (RtAbertoEstCod, RtAbertoNsu, RtAbertoAutCod, RtAbertoDataTrn, RtAbertoProduto, RtAbertoBandeira,
RtAbertoTrnCod, RtAbertoValorTrn, RtAbertoValorLIQEST, RtAbertoValor, RtAbertoDataInc, RtAbertoGuid)
SELECT @ESTCOD, A.MovTrnNsu, A.MovTrnAutCodStr, A.MovTrnDta, 
A.MovTrnTipPrd, A.MovTrnBan, A.MovTrnCod, A.MovTrnVlr, A.MovTrnVlrLiqEst, B.VALORABERTO, @DATAINC, @RELGUID
...
AND A.MovTrnAutCodStr = B.VlpAutCodStr
AND A.MovTrnCod <> B.VlpTrnCod;
/*ANALITICO VALOR ABERTO COM AJUSTE (V2) - FIM*/
...
RtAnaliticoValorPagoBNF, RtAnaliticoValorConc, RtAnaliticoUsr, RtAnaliticoDataInc, RtAnaliticoGuid)
SELECT 
...
B.VALORCESSAOORI, B.CUSTOANT, B.VALORCESSAOBNF, B.PAGOBNF, B.VALORCONCILIADO, @UsrId, @DATAINC, @RELGUID
